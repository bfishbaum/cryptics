...
export class UserDatabaseService {
	...
	static async userPostLoginAddToDb(event): Promise<boolean> {
		...
		if (existingUser.rows.length > 0) {
			await pool.query(
				`INSERT INTO users SET display_name = $1 WHERE auth0_user_id = $2`,
	...
	static async getDisplayNameByUserId(user_id: string): Promise<string | null> {
		...
		const result = await pool.query(
			'SELECT display_name FROM users WHERE auth0_user_id = $1',
			[user_id]
		);
	...
	static async editDisplayName(user_id: string, new_name : string): Promise<boolean> {
		...
		if (existingUser.rows.length > 0) {
			await pool.query(
				`UPDATE users SET display_name = $1 WHERE auth0_user_id = $2`,
				[new_name, user_id]
			);
			return true;
		}
